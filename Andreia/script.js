// ===== script.js — Client-side logic (Firebase/Firestore) =====
// Depends on firebase-config.js being loaded first

// ===== CART STATE =====
let cart = [];
let allProducts = []; // Cache of products from Firestore
...
commentForm.addEventListener('submit', async (e) => {
    ...
    const result = await addComment({ name, text, stars: selectedStars });
    ...
    if (result) {
        ...
        // Re-load testimonials from Firestore
        ...
        setTimeout(() => {
            ...
            submitCommentBtn.textContent = '✉️ Enviar Depoimento';
        }, 4000);
    } else {
...
orderWhatsApp.addEventListener('click', () => {
    ...
    let msg = '🍞 *Nova Encomenda — Pão da Andreia*\n\n📋 *Itens do pedido:*\n';
    cart.forEach(item => {
